...
void btree::insert(char *key, uint64_t val){
	...
	printf("current sizeof: %d\n", sizeof(current));
	if(sizeof(current) < PAGE_SIZE/2){ // 50% 이하로 용량이 채워져있을 땐 바로 삽입한다 (데이터 개수로 하는게 아니라 용량으로 해야해?)
		printf("50%이하\n");
		...
		if(new_child != NULL){ // split에 실패할 경우
			parent->set_leftmost_ptr(new_child); // parent의 오른쪽이 새로 만들어진 페이지를 가리키도록한다, 왼 쪽 child는 이미 만들어졌음
			insert(parent_key, (uint64_t)new_child); // parent key와 자식 주소를 부모 노드에 저장하기 위해 insert를 부른다 (또 split이 일어날 수 있기 때문에)
		}
